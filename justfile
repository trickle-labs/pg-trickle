...
[group: "build"]
build-hub:
    docker build -t pgtrickle/pg_trickle:0.108.0-pg18 -f Dockerfile.ghcr .
...
[group: "lint"]
lint: fmt-check clippy security-definer-check privilege-boundary-check docs-lint check-sql-builder check-fuzz-targets monitoring-contract
...
docs-lint:
    ...
    python3 scripts/check_error_identifiers.py
...
[group: "lint"]
lint-ci: lint check-version-sync check-meta-version check-stale-versions check-capability-manifest
    @echo "lint-ci passed — all CI lint checks satisfied"

# ROWID-17: Keep the V2 wire contract, recreation workflow, and release files aligned.
...
# Keep the v0.90 freshness-controller contract fail-closed.
v0-90-release-gate:
    python3 scripts/v0_90_release_gate.py

# Keep v0.91 schema-evolution behavior and release artifacts aligned.
v0-91-release-gate:
    python3 scripts/v0_91_release_gate.py

# Keep v0.92 backup, upgrade, clone, and CDC recovery contracts aligned.
v0-92-release-gate:
    python3 scripts/v0_92_release_gate.py
...
# Run the v0.104 public Graph and Delta conformance suite.
[group: "test"]
test-v104-conformance:
    bash ./scripts/run_light_e2e_tests.sh --package --test e2e_v104_conformance_tests

# Run the v0.108 Graph V1.2 and Delta V1.1 public conformance suite.
[group: "test"]
test-v108-conformance: build-e2e-image
...
dvm-composition-unit:
    ...
    cargo test --test e2e_dvm_composition_tests --features pg18 semantic_coverage_report_validator_accepts_complete_observation

# v0.87.5: run typed schema, snapshot plan, trace, and semantic-floor checks.
[group: "dvm"]
dvm-contracts-unit:
    ./scripts/run_unit_tests.sh pg18 dvm::schema
    ./scripts/run_unit_tests.sh pg18 dvm::snapshot
    ./scripts/run_unit_tests.sh pg18 dvm::diff::tests::test_decision_trace_records_declared_schema_and_snapshot_plan
    cargo test --test e2e_dvm_composition_tests --features pg18 semantic_coverage_report_validator_accepts_complete_observation

# v0.87.4: run state-directed and metamorphic correctness checks.
[group: "dvm"]
dvm-metamorphic: build-e2e-image
    ./scripts/run_e2e_tests.sh --test e2e_dvm_metamorphic_tests --no-capture

# v0.87.4: run pure planner and metamorphic model checks without Docker.
[group: "dvm"]
dvm-metamorphic-unit:
    cargo test --test e2e_dvm_metamorphic_tests --features pg18 dvm_fuzz::
    cargo test --test e2e_dvm_metamorphic_tests --features pg18 e2e::oracle::
    cargo test --test e2e_dvm_metamorphic_tests --features pg18 test_v0874_state_planner_covers_changed_leaf_buckets
    cargo test --test e2e_dvm_metamorphic_tests --features pg18 test_v08714_metamorphic_families_execute_and_record
...
[group: "upgrade"]
build-upgrade-image from="0.40.0" to="0.108.0": build-e2e-image
    ./tests/build_e2e_upgrade_image.sh {{from}} {{to}}

# Run upgrade E2E tests (builds base + upgrade Docker images first)
[group: "upgrade"]
test-upgrade from="0.7.0" to="0.108.0": (build-upgrade-image from to)
