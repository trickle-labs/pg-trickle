...
[group: "build"]
build-hub:
    docker build -t pgtrickle/pg_trickle:0.77.0-pg18 -f Dockerfile.hub .
...
[group: "upgrade"]
build-upgrade-image from="0.40.0" to="0.77.0": build-e2e-image
    ./tests/build_e2e_upgrade_image.sh {{from}} {{to}}

# Run upgrade E2E tests (builds base + upgrade Docker images first)
[group: "upgrade"]
test-upgrade from="0.7.0" to="0.77.0": (build-upgrade-image from to)
