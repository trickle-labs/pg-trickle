#!/usr/bin/env python3
"""Validate the v0.106.0 qualification contract and its fail-closed controls."""
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"v0.106.0 release gate failed: {message}")
...
def invoke_writer(case: str) -> tuple[int, str]:
    with tempfile.TemporaryDirectory(prefix=".v106-evidence-test-", dir=ROOT) as temp_name:
        ...
        artifact_path = temp / "pg_trickle-0.106.0-pg18-linux-amd64.tar.gz"
...
def check_suite_runner() -> None:
    with tempfile.TemporaryDirectory(prefix=".v106-suite-runner-", dir=ROOT) as temp_name:
        ...
        (package / "extension/pg_trickle.control").write_text("default_version = '0.106.0'\n", encoding="utf-8")
        artifact = temp / "pg_trickle-0.106.0-pg18-linux-amd64.tar.gz"
...
def main() -> None:
    ...
    require(contract.get("source_versions") == ["0.105.3"], "previous package boundary drift")
    ...
    require("v0_106_0_release_gate.py" in preflight, "v0.106.0 release gate is not wired into preflight")
    ...
    print("v0.106.0 qualification, evidence, publication, and negative-control gates passed")
