#!/usr/bin/env python3
"""Validate the v0.105.2 package and field qualification contract."""
...
VERSION = "0.105.2"
SOURCE_VERSION = "0.105.1"
QUALIFICATION = ROOT / "tests/release/v0.105.2-qualification.json"
EXPECTED_SUITES = {
    "graph-v1",
    "delta-v1",
    "dvm-oracle",
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"v0.105.2 release gate failed: {message}")
...
def read_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise SystemExit(f"v0.105.2 release gate failed: {path.relative_to(ROOT)}: {error}") from error
...
def main() -> None:
    ...
    require_test(ROOT / "tests/e2e_v08711_snapshot_security_tests.rs", "test_v08711_snapshot_restore_select_and_transfer_policy")
    require_test(ROOT / "tests/e2e_diagnostics_tests.rs", "test_diagnostics_migrate_reports_aligned_versions")

    budgets = qualification.get("performance_budgets")
    require(isinstance(budgets, list) and budgets, "performance_budgets must be non-empty")
    for budget in budgets:
        require(isinstance(budget, dict), "each performance budget must be an object")
        require(isinstance(budget.get("id"), str) and budget["id"].strip(), "performance budget id is missing")
        require(isinstance(budget.get("threshold"), (int, float)) and budget["threshold"] >= 0, f"performance budget {budget.get('id')!r} has an invalid threshold")
    ...
    for relative in ("docs/RELEASE.md", "docs/UPGRADING.md", "docs/ROLLBACK_RUNBOOK.md", "docs/OPS_CHEATSHEET.md"):
        require((ROOT / relative).is_file(), f"operator procedure documentation is missing {relative}")
    ...
    print("v0.105.2 release gate passed")
