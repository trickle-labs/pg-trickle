...
Publication creation and drop are caller-context operations for public
PostgreSQL DDL. The caller must own the stream table and hold `CREATE` on the
current database. PostgreSQL assigns the resulting publication to that caller.
Stream ownership does not bypass the database privilege check.

The private publication binding is committed with the public DDL. Its live
publication OID, owner, stream relation, and relation set must match before a
drop or repair proceeds. Name reuse, rename, ownership transfer, and relation
set drift fail closed. Publication APIs do not expose private catalog or
change-buffer privileges.
