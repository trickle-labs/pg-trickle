...
fn apply_ivm_owner_delta(
    ...
    let delta_table_quoted =
        crate::refresh::with_stream_owner(st, || {
            crate::refresh::prepare_owner_temp_table(st, delta_table, delta_sql)
        })?;
...
fn pgt_ivm_apply_delta(
    ...
    // only capture NEW-row visibility, not whether an OLD row image was
    // ever visible to the stream owner, so applying this delta could
    // silently produce a wrong result — refuse instead.
    if let Some(rls_source) = crate::cdc::first_rls_enabled_source(&[source_oid_u32])? {
        return Err(PgTrickleError::UnsupportedOperator(format!(
            "IMMEDIATE-mode stream table {}.{} cannot apply this change: source \"{}\" \
             now has row-level security enabled. Switch the stream table to 'FULL' or \
...
fn apply_topk_micro_refresh(st: &crate::catalog::StreamTableMeta) -> Result<(), PgTrickleError> {
    ...
    // topk_select is definition-derived: parse/analyze it under the owner's
    // identity and stored search_path, not this privileged caller's.
    let new_topk = crate::refresh::with_stream_owner(st, || {
        crate::refresh::prepare_owner_temp_table(st, &new_topk_basename, &topk_select)
    })?;
    let result = crate::refresh::with_stream_owner(st, || {
