...
pub fn execute_differential_refresh_with_tuning(
    ...
    let mut resolved = if let Some(entry) = cached {
        ...
        //
        // EC-06: Keyless storage uses counted DELETE + plain INSERT.
        // Delta deduplication does not make the target row-id index unique.
        let use_keyless = st.has_keyless_source;
        ...
        let trigger_insert_template =
            if use_keyless && (is_dedup || user_cols.iter().any(|c| c == "__pgt_count")) {
    ...
    let prepared_diff_capture_cols = if use_explicit_dml && needs_downstream_capture {
        ...
        if !cols.is_empty() {
            ...
            let pre_select = format!(
                "SELECT st.__pgt_row_id, {col_list} FROM {quoted_table} st \
            ...
            let del_count = if !st.has_keyless_source {
                ...
                let unified_del_sql = format!(
                    "DELETE FROM {quoted_table} AS st \
                     USING (SELECT DISTINCT __pgt_row_id FROM __pgt_delta_{pgt_id}) AS d \
                     WHERE pgtrickle.row_probe_v1(st.__pgt_row_id) = pgtrickle.row_probe_v1(d.__pgt_row_id) \
                       AND st.__pgt_row_id = d.__pgt_row_id",
                    pgt_id = st.pgt_id,
                );
