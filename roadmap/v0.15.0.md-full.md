...
| Item | Description | Effort | Ref |
|------|-------------|--------|-----|
| WM-7 | **Watermark hold-back mode.** `watermark_holdback_timeout` GUC detects stuck watermarks; pauses downstream gated STs; emits `pgtrickle_alert` NOTIFY with `watermark_stuck` event; auto-resumes with `watermark_resumed` event when watermark advances. | ✅ Done | Implemented watermark-gating behavior, §4.1 |
...
| Item | Description | Effort | Ref |
|------|-------------|--------|-----|
| E5 | ORM integrations guide (SQLAlchemy, Django, etc.) | 8–12h | Completed documentation scope |
...
| Item | Description | Effort | Ref |
|------|-------------|--------|-----|
| E4 | Flyway / Liquibase migration support | 8–12h | Completed documentation scope |
